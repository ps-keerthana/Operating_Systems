...
static struct proc* allocproc(void)
{
    ...
    *(uint*)sp = (uint)p->kstack + KSTACKSIZE;
    ...
    p->count_sc = 0; // setting no of systemcalls to zero when the process gets alloted initially

    // --- Add lottery scheduler defaults ---
    if(proc->parent){
...
// function for how custom proclist systemcall
int proclist(int max, struct user_proc_data *list)
{
  struct proc *p; // a pointer to iterate in process table
  int count = 0; // variable to count no of process have been added to output list
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC] && count < max; p++){
    ...
    //fill in the user_proc_data structure for each process and increment count
    list[count].pid = p->pid;
    list[count].ppid = p->parent ? p->parent->pid : 0;
    safestrcpy(list[count].name, p->name, sizeof(list[count].name));
    list[count].state = p->state;
    list[count].count_sc = p->count_sc;
    count++;
  }
  release(&ptable.lock); // release process table lock
...
// Choose a process via lottery
struct proc* hold_lottery(int total_tickets) {
  if (total_tickets <= 0) {
    cprintf("hold_lottery: no tickets\n");
    return 0;
  }

  uint r = rand();
  uint winner_ticket = r % total_tickets;
  int counter = 0;
    ...
    for(;;){
        ...
        if(total_tickets > 0) {
            ...
            if(winner) {
                ...
                winner->runticks++;             // increment here
...
// copy process info into user’s pstat
int
