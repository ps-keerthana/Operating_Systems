...
#include "arm.h"
#include "proc.h"
#include "mmu.h"      // for align_dn, PTE_SZ, AP_KU, PE_TYPES, etc.
#include "memlayout.h"
...
// extern uint V2P(void *va);
extern void free_page(void *v);
...
void
dabort_handler(struct trapframe *r)
{
    uint dfs, fa;
    struct proc *p = proc;
    int ret;

    cprintf("DABORT: 1. Handler started.\n");
    ...
    if (p && fa < p->sz) {
        cprintf("DABORT: 2. Fault is valid. Addr=0x%x\n", fa);
        
        char *fault_addr = (char*) align_dn(fa, PGSIZE);

        extern pte_t* walkpgdir(pde_t *pgdir, const void *va, int alloc);
        pte_t *pte = walkpgdir(p->pgdir, fault_addr, 1);
        cprintf("DABORT: 3. walkpgdir finished.\n");

        if (pte && (*pte & PE_TYPES)) {
            flush_tlb();
            sti(); 
            cprintf("DABORT: 3a. Page already mapped, returning.\n");
            return;
        }

        char *mem = alloc_page();
        if (mem == NULL) {
            panic("out of memory in dabort_handler");
        }
        cprintf("DABORT: 4. alloc_page finished.\n");
        
        memset(mem, 0, PGSIZE);

        ret = mappages(p->pgdir, fault_addr, PGSIZE, V2P(mem), AP_KU);
        if (ret != 0) {
            free_page(mem);
            panic("dabort_handler: mappages failed");
        }
        cprintf("DABORT: 5. mappages finished.\n");

        flush_tlb();
        sti(); 
        cprintf("DABORT: 6. Handler finished, about to return.\n");
        return;
    }
