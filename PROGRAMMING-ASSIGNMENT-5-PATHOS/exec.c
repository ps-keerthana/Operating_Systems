...
int exec (char *path, char **argv)
{
    ...
    // --- ADD THIS BLOCK TO PRE-ALLOCATE THE STACK ---
    // It eagerly allocates just one physical page for the stack.
    char *mem;
    if ((mem = alloc_page()) == 0) {
        goto bad;
    }
    memset(mem, 0, PGSIZE);
    if (mappages(pgdir, (void*)(sz - PTE_SZ), PGSIZE, V2P(mem), AP_KU) < 0) {
        free_page(mem);
        goto bad;
    }
// --- END OF BLOCK ---
