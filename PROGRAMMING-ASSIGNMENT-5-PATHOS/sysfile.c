...
int sys_symlink(void) {
    ...
    // Find parent directory inode and name for the new symlink
    if ((dp = nameiparent(path, name)) == 0) {
        return -1;
    }
    ...
    // Allocate a new inode for symlink
    ...
    // Write the symlink target path string into symlink inode's data
    if (writei(ip, target, 0, ip->size) != ip->size) {
        // Failed to write target, cleanup
        ip->nlink = 0;
        iupdate(ip);
        iunlockput(ip);
        iunlockput(dp);
...
int sys_open(void)
{
    ...
    if(omode & O_CREATE){
        ...
        while (ip->type == T_SYMLINK) {
            ...
            target[n] = '\0'; // Null-terminate the target string
