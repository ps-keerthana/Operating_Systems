...
static uint bmap(struct inode *ip, uint bn)
{
    ...
    if (bn < NDOUBLEINDIRECT) {
        ...
        uint idx1 = bn / NINDIRECT;  // index in doubly indirect block
        uint idx2 = bn % NINDIRECT;  // index in singly indirect block
        ...
        if (a[idx1] == 0) {
            ...
            log_write(bp);
            brelse(bp);
            // Read again for new singly indirect block
            bp = bread(ip->dev, addr);
            a2 = (uint *)bp->data;
        } else {
            brelse(bp);
            bp = bread(ip->dev, a[idx1]);
            a2 = (uint *)bp->data;
        }
...
static void itrunc (struct inode *ip)
{
    ...
    // Free doubly indirect blocks (THIS IS THE MISSING PART!)
    if (ip->addrs[NDIRECT + 1]) {
        bp = bread(ip->dev, ip->addrs[NDIRECT + 1]);
        a = (uint*) bp->data;

        // Iterate through each singly indirect block pointer
        for (j = 0; j < NINDIRECT; j++) {
            if (a[j]) {
                bp2 = bread(ip->dev, a[j]);
                a2 = (uint*) bp2->data;

                // Free all data blocks in this singly indirect block
...
int writei (struct inode *ip, char *src, uint off, uint n)
{
    ...
    // Overflow/sanity checks (unchanged)
    if (off > ip->size || off + n < off) {
        return -1;
    }

    // Max allowed according to block numbers, with updated MAXFILE
    if (off + n > MAXFILE * BSIZE) {
        return -1;
    }

    for (tot = 0; tot < n; tot += m, off += m, src += m) {
        // bmap now handles direct, indirect, and doubly-indirect
        bp = bread(ip->dev, bmap(ip, off / BSIZE));
        m = min(n - tot, BSIZE - off % BSIZE);
...
int symlink_write_target(struct inode *ip, const char *target) {
    int len = strlen(target);
    if (len > MAXFILE * BSIZE) {
        // Target path too long for symlink content blocks
        return -1;
    }

    // Write the symlink target path into the inode's data blocks
    int res = writei(ip, (char *)target, 0, len);
    if (res != len) {
        return -1;
    }
