...
// --- Ensure we use PGSIZE everywhere ---
#ifndef PGSIZE
#define PGSIZE 4096
#endif
...
int mappages (pde_t *pgdir, void *va, uint size, uint pa, int ap)
{
    ...
    a = (char*) align_dn(va, PGSIZE);
    last = (char*) align_dn((uint)va + size - 1, PGSIZE);
...
void inituvm (pde_t *pgdir, char *init, uint sz)
{
    ...
    mem = alloc_page();
    memset(mem, 0, PGSIZE);
    mappages(pgdir, 0, PGSIZE, V2P(mem), AP_KU);
    memmove(mem, init, sz);
}
...
loaduvm(pde_t *pgdir, char *addr, struct inode *ip, uint offset, uint sz)
{
    ...
    for (i = 0; i < sz; i += PGSIZE) {
        ...
        memset(mem, 0, PGSIZE);
        if (mappages(pgdir, addr + i, PGSIZE, V2P(mem), AP_KU) < 0) {
            free_page(mem);
            deallocuvm(pgdir, sz, 0);
            return -1;
        }
...
int deallocuvm (pde_t *pgdir, uint oldsz, uint newsz)
{
    ...
    for (a = align_up(newsz, PGSIZE); a < oldsz; a += PGSIZE) {
...
copyuvm(pde_t *pgdir, uint sz)
{
    ...
    for (i = 0; i < sz; i += PGSIZE) {
        ...
        // If page isn't physically present, it's an on-demand page. Skip it. before it used to panic if it comes across unassigned pages
        ...
        if (mappages(d, (void*)i, PGSIZE, V2P(mem), ap) < 0) {
            free_page(mem);
            goto bad;
        }
    }
